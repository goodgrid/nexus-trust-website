...
A second, live case proves build-operate-transfer and sovereignty in practice: for a client-identification service in a regulated sector, Nexus Trust moved the service onto European infrastructure, took its operation and development in-house from an external integrator (build, operate, and soon hand over), and is replacing the procured black-box ID-authenticity software with a modular, open-source-based setup that is fully under control - except the authenticity engine itself, which is isolated and contained - for higher reliability and control. (Sector omitted; external integrator not named; keep neutral - see guardrail in §10.)
